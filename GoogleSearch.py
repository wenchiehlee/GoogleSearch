...
Description:
    ...
    v2.0.6 adds configurable search parameters and individual search enable/disable options.
...
def google_search(api_key, cse_id, query, num_results=10, max_results=500, last_days=None, language=None, country=None):
    ...
    print(f"🔍 Search parameters: days={last_days}, lang={language}, country={country}")
...
def main():
    ...
    search_queries = [
        ...
        },
        {
            ...
            "search_params": {
                "last_days": 360,    # Remove date restriction
                "language": "lang_zh-TW",     # Remove language restriction
                "country": None       # Remove country restriction
            }
        }
    ]
    ...
    if not enabled_searches:
        print("❌ No searches are enabled! Please set at least one search to 'enabled': True")
        print("💡 Edit the ENABLE_* variables at the top of main() function")
        return
    
    print(f"🎯 Running {len(enabled_searches)} of {len(search_queries)} configured searches:")
    ...
    for disabled_search in [s for s in search_queries if not s.get("enabled", True)]:
        print(f"   ⏸️  {disabled_search['description']} (disabled)")
